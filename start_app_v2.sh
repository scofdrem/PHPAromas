...
start_services_with_retry() {
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        # Start backend service using Laravel artisan serve
        log_info "Starting Laravel Backend service on port $BACKEND_PORT..."
        php artisan serve --host=0.0.0.0 --port=$BACKEND_PORT &
        BACKEND_PID=$!

        # Wait for backend to start
        log_info "Waiting for Laravel Backend service to start..."
        sleep 5

        # Check if backend started successfully
        if ! kill -0 $BACKEND_PID 2>/dev/null; then
            log_warning "Backend startup attempt $retry_count failed"
            # Clean up failed backend process
...
main() {
    ...
    if [ -f "composer.json" ]; then
        if command -v composer >/dev/null 2>&1; then
            ...
            log_warning "Composer not found and vendor directory missing. Please install Composer and run 'composer install'"
        fi
    else
        log_warning "composer.json not found in $BACKEND_DIR"
    fi
