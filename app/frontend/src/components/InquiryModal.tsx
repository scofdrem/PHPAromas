...
export default function InquiryModal({ product, selectedVolume, isOpen, onClose }: InquiryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await laravelApi.createInquiry({
        name: customerName.trim() || "Не указано",
        phone: telegram.trim() || customerEmail.trim(),
        message: message.trim() || `Запрос на ${product.name} (${product.brand}), ${selectedVolume} мл`,
